...
Edge-Runtime-configured sibling to [`examples/nextjs`](../nextjs): the same routes (login, callback, protected account route, full logout, transparent token refresh), but built on `ScalekitEdgeClient` from `@scalekit-sdk/node/edge` (fetch + `jose`, no gRPC, no Node-only APIs) instead of the default gRPC-based `ScalekitClient`, so `middleware.ts` can pin `runtime: 'edge'` instead of falling back to `runtime: 'nodejs'`. A real `npx next build` against this app is the proof that `ScalekitEdgeClient` actually compiles clean for Vercel/Next.js Edge Runtime.
...
- `middleware.ts` — `runtime: 'edge'` pinned explicitly (not the `nodejs` fallback the sibling example needs).
