 ...
 */
function hasTransportResetCause(error) {
    const seen = new Set();
    let current = error;
...
class ScalekitServerException extends ScalekitException {
    constructor(error) {
        ...
        if (this.isAxiosResponse(error)) {
            ...
            if (error.statusText && typeof error.statusText === 'string') {
                this._httpStatus =
                    HTTP_STATUS[error.statusText.toUpperCase()] || HTTP_STATUS.INTERNAL_SERVER_ERROR;
            }
            else {
            ...
            this._errDetails = error.findDetails(errdetails_pb_1.ErrorInfoSchema);
    ...
    static promote(error, isToolError) {
        ...
        if (error instanceof connect_1.ConnectError &&
            grpcStatus === connect_1.Code.Aborted &&
            hasTransportResetCause(error)) {
            const transientError = new connect_1.ConnectError(error.rawMessage, connect_1.Code.Unavailable, error.metadata, undefined, error.cause);
        ...
        const toolError = isToolError !== null && isToolError !== void 0 ? isToolError : (error instanceof connect_1.ConnectError &&
            error
